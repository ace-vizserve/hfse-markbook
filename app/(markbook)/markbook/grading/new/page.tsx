...
export default async function NewGradingSheetPage() {
  ...
  if (!sessionUser || !sessionUser.role || !ALLOWED_ROLES.has(sessionUser.role)) {
    notFound();
  }
  ...
  return (
    <PageShell>
      ...
      <NewSheetForm
        ...
        sections={(sectionsRes.data ?? []) as Parameters<typeof NewSheetForm>[0]["sections"]}
  ...
  return (
    <PageShell>
      ...
      <header className="space-y-4">
        ...
        <p className="max-w-2xl text-[15px] leading-relaxed text-muted-foreground">{message}</p>
      </header>
    </PageShell>
  );
}
