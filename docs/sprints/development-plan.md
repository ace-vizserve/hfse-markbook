...
- **`getSheetReadinessBySection` + `getTeacherEntryVelocity`** (deferred 2026-04-24, Sprint 21) — drill targets `sheet-readiness-section` + `teacher-entry-velocity` are defined in `lib/markbook/drill.ts` + `app/api/markbook/drill/[target]/route.ts` as of 2026-04-25, but no surface card on `/markbook` yet. Adding a card is ~50 lines (Card + ComparisonBarChart wrapped in a per-target client wrapper). Teacher-velocity is privacy-gated to registrar+ in the API route.
- **`getAttendanceBySectionRange` + `getCompassionateQuotaUsage`** (deferred 2026-04-24, Sprint 21) — drill targets `attendance-by-section` + `compassionate-quota` are defined in `lib/attendance/drill.ts` + `app/api/attendance/drill/[target]/route.ts` as of 2026-04-25. Roll-up logic lives in `rollupBySection` + `rollupCompassionate`. Surface cards on `/attendance` deferred — same ~50-line pattern as above.
