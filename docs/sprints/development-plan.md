...
## Status snapshot (last updated 2026-05-25, **Sprint 42 — teacher-owned SOW model**: migration 061 shipped — reshape `sow_class_instances` to teacher-owned `(ww_labels, pt_labels, topics, copied_from_section_id, copied_at, created_by)` jsonb columns; re-scope `evaluation_checklist_items` back to `section_id`; drop `sync_grading_sheets_from_sow` RPC. SOW authoring at `/markbook/sow` (teacher index + `[sectionId]/[subjectId]/[termId]` editor); import from peer section stamps provenance. `/sis/admin/sow` converted to coordinator read-only review (`<SowReviewTable>`). AY Readiness Pill trimmed to 4 steps (SOW step removed — no longer a gate). Evaluation checklist CRUD un-410'd. 4 new audit actions: `sow.instance.save`, `sow.instance.import_from`, `sow.labels.synced`, `sow.topics.synced`. `sow_master_templates` + `sow_published_versions` vestigial (cleanup migration deferred). TypeScript clean; build 120 pages. KD #110 documents the teacher-owned model. PRECEDED BY **Sprint 41 — SOW hard gate + AY readiness**: migrations 059–060 shipped — (059) `curriculum_track` on `template_sections` + `apply_template_to_ay` RPC fix; (060a) `sow_class_instances.has_partial_rebaseline` + `create_grading_sheets_for_scopes` RPC; (060b) `sow_subject_scopes` table seeded for P1–P6 + S1–S4; SOW builder simplified to section-scoped single Save & Apply flow with Subject Scope Manager; AY Readiness Pill (5-step) in SIS layout visible to school_admin+ (KD #109); SIS hub redesigned with Year Setup section + numbered AdminCards; evaluation-checklists admin page removed — evaluation topics fully SOW-sourced (KD #108). PRECEDED BY **Sprint 40 — SOW builder + attribution**: migration 058 shipped — `sow_master_templates`, `sow_published_versions`, `sow_class_instances` tables + `sections.curriculum_track`; SOW builder at `/sis/admin/sow` (school_admin+); grading sheets display SOW attribution; SIS Admin audit log extended. PRECEDED BY **Sprint 39 — slot metadata + evaluation subject-teacher nav**: migration 057 shipped. Evaluation sections picker widened to subject-teacher assignments (KD #106). Evaluation design discovery: topics admin-prescribed per `(subject × level × term)` (KD #107 — later superseded by KD #110). PRECEDED BY **Sprint 38+ — post-demo production refinement**: migrations 050–056 shipped (KDs #96–105). PRECEDED BY **Sprint 38 — workflow hardening**: migrations 044–049 (KDs #88–95). PRECEDED BY **Sprint 37 — demo-readiness**: unified data-tables (KD #84) + direct-create user (KD #87). Full sprint detail in git log + sprint sections below.
...
### Sprint 39 — Slot metadata + evaluation subject-teacher nav ✅ Done (2026-05-20)

**Scope:** Two independent deliverables: (1) WW/PT slot metadata extension bringing the slot descriptor surface to parity with Joann's Excel workbook columns; (2) evaluation sections picker navigation fix so subject teachers can reach their sections.

#### (a) Slot metadata — migration 057 + UI (KD #105)

- [x] Migration 057 — one-shot coerces existing `slot_labels` string entries to `{ label, date, page }` objects; null entries stay null; `qa` string unchanged
- [x] Gate updated: both `label` AND `date` required to unlock score cells; previously-scored cells in a labeled-but-undated slot stay editable
- [x] `ActivityLabelsForm` / `LabelRow` — 3-column layout (Description · Page # · Date administered) + "Use today" quick-button
- [x] `SlotChip` — conditional display omitting empty fields; muted styling when label set but date not
- [x] `saveLabel` / `PATCH /api/grading-sheets/[id]/labels` — `Partial<SlotMeta>` merge semantics so per-field blur doesn't clobber other fields
- [x] `SlotMetaPatchSchema` in `lib/schemas/grading-sheet.ts` with per-field sanitizers (label ≤120, page ≤40, date ISO-only)
- [x] Publish-readiness soft-warning row: `slot_dates` block in `GET /api/sections/[id]/publish-readiness` cross-references `grade_entries.ww_scores/pt_scores` against `slot_labels.date`; `CalendarDays` amber `ChecklistRow` in `components/admin/publish-window-panel.tsx`

#### (b) Evaluation subject-teacher navigation (KD #106)

- [x] `listSubjectTeacherSectionIds(userId)` helper in `lib/evaluation/queries.ts`
- [x] `getChecklistTopicCountByTerm(termId, sectionIds)` helper in `lib/evaluation/queries.ts`
- [x] `/evaluation/sections` picker fetches adviser + subject-teacher sets in parallel; filter widened to union
- [x] Role-differentiated cards: adviser cards show write-up progress bar; subject-teacher-only cards show topic count + student count
- [x] Write-up stats strip hidden for pure subject teachers
- [x] Hero copy, empty state, card eyebrow updated with role context

#### (c) Evaluation design discovery (KD #107 — confirmed, unblocked for implementation)

- [x] Field investigation confirmed topics are admin-prescribed per `(subject × level × term)`, not teacher-owned per section
- [x] Chandana confirmed: WW/PT activity labels are also centrally prescribed — unblocks SOW builder (KD #108)
- [ ] Migration to revert `evaluation_checklist_items.section_id` → `level_id + curriculum_track` — _unblocked, planned for next sprint_
- [ ] PTC comment encoding surface for teachers — _still pending: need confirmation of who encodes_

---

### Sprint 42 — Teacher-owned SOW model ✅ Done (2026-05-25)

**Scope:** Flip SOW ownership from admin-authoring model (KD #108) to teacher-owned model (KD #110). HFSE's actual workflow: subject teachers finalize the SOW off-system and enter it into the SIS; coordinators do light random spot-checks. Removes the hard gate; grading-sheet creation no longer depends on SOW being published.

#### (a) Schema — migration 061

- [x] `sow_class_instances` reshaped: drop `master_template_id`, `version_id`, `has_partial_rebaseline`; add `ww_labels jsonb NOT NULL DEFAULT '[]'`, `pt_labels jsonb NOT NULL DEFAULT '[]'`, `topics jsonb NOT NULL DEFAULT '[]'`, `copied_from_section_id uuid REFERENCES sections(id) ON DELETE SET NULL`, `copied_at timestamptz`, `created_by uuid REFERENCES auth.users(id) ON DELETE SET NULL`; re-assert UNIQUE `(section_id, subject_id, term_id)`
- [x] `evaluation_checklist_items` re-scoped: TRUNCATE (safe — all writes 410'd since migration 058); drop `level_id`, `curriculum_track`, `sow_class_instance_id`; add `section_id NOT NULL REFERENCES sections(id) ON DELETE CASCADE`, `sow_instance_id uuid REFERENCES sow_class_instances(id) ON DELETE SET NULL`; recreate unique index + scope index on `(term_id, subject_id, section_id, sort_order)`
- [x] Drop `sync_grading_sheets_from_sow` RPC (no longer needed)

#### (b) Teacher SOW authoring surface at `/markbook/sow`

- [x] `lib/markbook/sow.ts` — `listTeacherSowItems(userId, ayCode, isRegistrarPlus)` + `syncSowLabelsToSheet` + `syncSowTopicsToChecklist`
- [x] `app/(markbook)/markbook/sow/page.tsx` — index: groups by level, status badge (empty / drafted / synced), links to editor
- [x] `app/(markbook)/markbook/sow/[sectionId]/[subjectId]/[termId]/page.tsx` — RSC editor (resolves section/subject/term metadata, existing instance, importable sources, grading sheet lock state)
- [x] `components/markbook/sow-editor-client.tsx` — client editor: WW labels + page, PT labels + page (max 5 each per Hard Rule #5), evaluation topics (add/reorder/remove), Save (PUT), Import from peer section, Sync labels to grading sheet (disabled if locked), Seed eval topics; provenance banner when `copied_from_section_id` set
- [x] Markbook sidebar: "Planning" nav group with "Scheme of Work" entry (`ScrollText` icon); visible to teacher/registrar/school_admin/superadmin

#### (c) Coordinator review surface at `/sis/admin/sow`

- [x] `components/sis/sow-review-table.tsx` — read-only table: term + subject pickers, one row per section (advisor / WW labels / PT labels / topic count / copied-from / last edited / grading sheet status); no write actions
- [x] `app/(sis)/sis/admin/sow/page.tsx` — rewritten to mount `<SowReviewTable>` alongside existing `<SowScopeManager>`
- [x] `app/api/sis/admin/sow/route.ts` — GET refactored (`getSowReviewRows`); PUT removed
- [x] Deleted `app/api/sis/admin/sow/apply/route.ts` + `app/api/sis/admin/sow/publish/route.ts` + `components/sis/sow-builder.tsx`

#### (d) API routes

- [x] `GET|PUT /api/sow` — fetch or upsert teacher's SOW for `(section × subject × term)`; auth: teacher assigned to section+subject OR registrar+
- [x] `POST /api/sow/import` — copy `ww_labels + pt_labels + topics` from peer section (same level × subject), stamp provenance; returns full data for immediate UI refresh
- [x] `POST /api/sow/[id]/sync-to-grading-sheet` — push labels into matching grading sheet; 423 if locked (Hard Rule #5)
- [x] `POST /api/sow/[id]/sync-to-eval` — seed `evaluation_checklist_items` from SOW topics (upsert, preserves existing)
- [x] Evaluation checklist CRUD un-410'd: `POST /api/evaluation/checklist-items`, `PATCH|DELETE /api/evaluation/checklist-items/[id]`

#### (e) Hard-gate removal + pipeline changes

- [x] `POST /api/grading-sheets/bulk-create` — drop `gateAndActivateScopes`; after sheet creation, call `syncSowLabelsToSheet` (soft — logs if SOW missing, does not fail)
- [x] `lib/sis/readiness.ts` — drop `checkSow` step; `getAyReadiness` now returns 4 steps
- [x] `components/sis/ay-readiness-pill.tsx` — 5 → 4 steps (SOW step removed; Grading Sheets renumbered to step 4)
- [x] `app/(evaluation)/evaluation/sections/[sectionId]/page.tsx` — SOW lock screen removed; `teacherCanEditTopics` derives from teacher assignment
- [x] `components/grading/score-entry-grid.tsx` — `sowSourced` prop removed; label fields always teacher-editable
- [x] `app/(markbook)/markbook/grading/[id]/page.tsx` — SOW props removed

#### (f) Audit trail

- [x] `lib/audit/log-action.ts` — 4 new `AuditAction` values: `sow.instance.save`, `sow.instance.import_from`, `sow.labels.synced`, `sow.topics.synced`
- [x] SIS audit-log allowlist: `sow.instance.save`, `sow.instance.import_from`, `sow.labels.synced`, `sow.topics.synced`
- [x] Markbook audit-log allowlist: same 4 actions
- [x] Evaluation audit-log allowlist: `sow.instance.save`, `sow.instance.import_from`, `sow.topics.synced` + `evaluation.checklist_item.*`

---

### Sprint 40 — SOW builder + attribution ✅ Done (2026-05-20)

**Scope:** Implement the Scheme of Work (SOW) Definition/Version/Instance model (KD #108). Chandana's earlier clarification that WW/PT labels + evaluation topics are centrally prescribed fully unblocked this sprint.

#### (a) Schema — migration 058 (KD #108)

- [x] `sections.curriculum_track text NOT NULL DEFAULT 'singapore_inspired'` — `cambridge | o_level | singapore_inspired`; CS* sections auto-set to `cambridge` via UPDATE
- [x] `sow_master_templates` — Chandana's editable draft per `(ay × term × subject × level × curriculum_track)`, UNIQUE constraint on the 5-tuple
- [x] `sow_published_versions` — immutable snapshots; `version_number` integer, UNIQUE per `(master_id, version_number)`
- [x] `sow_class_instances` — per-section binding to a published version; UNIQUE per `(section_id, subject_id, term_id)`
- [x] `evaluation_checklist_items` scope reverted from `(section × subject × term)` to `(level × curriculum_track × subject × term)` — topics are admin-prescribed, teachers read-only
- [x] Audit actions: `sow.master.upsert`, `sow.version.publish`, `sow.instance.apply`

#### (b) SOW builder UI at `/sis/admin/sow` (school_admin | superadmin)

- [x] Scope picker (AY → Term → Subject → Level → Curriculum Track) with cascading selects
- [x] Three builder sections: Evaluation Topics (ordered list with add/remove/drag-reorder) + Written Works (W1–W5 label + page) + Performance Tasks (PT1–PT5 label + page)
- [x] Slot row counts driven by `subject_configs.ww_max_slots / pt_max_slots` (not hardcoded 5)
- [x] Save draft (master upsert) + Publish (creates immutable snapshot, version-numbered)
- [x] Version history panel with "Apply this version to unlocked class instances" action

#### (c) Grading sheet + evaluation SOW version attribution

- [x] `sowVersion?: number | null` prop threaded into `<ScoreEntryGrid>` → `<ScoringGuide>` via grading sheet page
- [x] Attribution chip in compact reference strip: `SOW v{N}` when SOW-sourced; `No SOW applied` muted hint otherwise
- [x] `sowExistsForSection()` query result `.version?.version_number` passed as `sowVersion` prop from grading page
- [x] Evaluation `TopicManagerPanel` header shows `· SOW v{N}` when `sowVersionNumber` is set
- [x] `sowVersionNumber` threaded from evaluation sections page → `ChecklistRosterClient` → `TopicManagerPanel`

#### (d) Subject weights safety warning

- [x] Amber inline warning in `<SubjectConfigEditDialog>` when user reduces `ww_max_slots` or `pt_max_slots` below current saved value — notes that scores in removed slots will be lost and SOW labels beyond the new limit won't appear on future sheets

#### (e) SIS Admin audit log coverage

- [x] `SIS_AUDIT_ALLOWLIST` in `app/(sis)/sis/audit-log/page.tsx` extended with SOW actions: `sow.master.upsert`, `sow.version.publish`, `sow.instance.apply`
- [x] `SIS_AUDIT_ALLOWLIST` extended with school calendar actions: `attendance.calendar.upsert`, `attendance.calendar.delete`, `attendance.calendar.copy_from_prior_ay`
