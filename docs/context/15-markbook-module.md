...
| Term-summary attendance **entry** (`/admin/sections/[id]/attendance` route + `components/admin/attendance-grid.tsx`). **Table `attendance_records` stays** — consumed by Markbook for report cards, written by Attendance as the rollup target. | **Attendance** module (planned, daily-level) | Daily attendance will own the raw ledger; term-summary is a rollup. Markbook keeps the *report-card rendering* of attendance (totals on the card). The entry grid + route move; the table is shared. See `16-attendance-module.md` for the agreed frame. |
