  ...
  return (
    <div
      ...
      <div
        ...
        style={{
          backgroundImage: "url('/report-card/reference/report-card-header-bg.png')",
          backgroundSize: "100% 100%",
        }}>
        {config.organizationName && (
          <p className="font-sans text-[14px] font-bold leading-tight text-white">{config.organizationName}</p>
        )}
        {config.addressLine1 && <p className="text-[10px] leading-tight text-white">{config.addressLine1}</p>}
        {config.addressLine2 && <p className="text-[10px] leading-tight text-white">{config.addressLine2}</p>}
        ...
        {showPei && (
          <>
            <p className="text-[10px] leading-tight" style={{ color: "rgba(255,255,255,0.92)" }}>
              {config.peiRegistrationNumber && <>PEI Registration No. {config.peiRegistrationNumber}</>}
            </p>
