...
export function VacationAllowanceInline({
  ...
  async function persist(vlAllowance: number | null) {
    setSaving(true);
    try {
      const res = await fetch(`/api/sis/students/${encodeURIComponent(enroleeNumber)}/vl-allowance`, {
        method: "PATCH",
      ...
      if (vlAllowance === null) {
        toast.success(`Reset — now using school default (${schoolDefault} per term)`);
      } else {
        toast.success(
    ...
    <div className="flex flex-wrap items-center gap-3 rounded-xl border border-border bg-background p-4">
      <div className="flex-1 min-w-[220px]">
        ...
        <p className="mt-0.5 text-[12px] text-muted-foreground">
          ...
          {hasOverride && (
            <span className="ml-1 font-medium text-foreground">· Custom override</span>
          )}
      ...
      <div className="flex items-center gap-2">
        ...
        />
        <span className="font-mono text-[11px] text-muted-foreground">days / term</span>
        <Button
          type="button"
          variant="outline"
          size="sm"
