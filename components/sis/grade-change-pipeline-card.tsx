...
const PIPELINE_ROWS: Array<{
...
}> = [
  {
    ...
    icon: GitBranch,
    tileClass:
      'bg-gradient-to-b from-brand-indigo/20 to-brand-navy/10 text-brand-indigo',
  },
  {
    key: 'approved',
    label: 'Approved',
    ...
    icon: CheckCircle2,
    tileClass:
      'bg-gradient-to-b from-brand-mint/20 to-chart-5/10 text-brand-mint',
  },
  {
    key: 'rejected',
    label: 'Rejected',
    ...
    icon: XCircle,
    tileClass:
      'bg-gradient-to-b from-destructive/20 to-destructive/10 text-destructive',
  },
  {
    key: 'applied',
    label: 'Applied',
    ...
    icon: Zap,
    tileClass:
      'bg-gradient-to-b from-brand-mint/30 to-chart-5/15 text-brand-mint',
  },
  {
    ...
    icon: RotateCcw,
    tileClass:
      'bg-gradient-to-b from-brand-amber/20 to-brand-amber/10 text-brand-amber',
  },
];

export function GradeChangePipelineCard({
  pipeline,
}: {
  pipeline: GradeChangePipeline;
}) {
      ...
      <CardContent className="p-0">
        {!hasActivity ? (
          ...
          <ul className="divide-y divide-hairline">
            {PIPELINE_ROWS.map(
              ({ key, label, description, icon: Icon, tileClass }) => (
