...
**Current state (2026-05-06):** Sprint 34 shipping pre-demo — drill-empty + dashboard correctness + AY-scoping. Sprint root cause (`f73b448`) was PostgREST silently capping single-query responses at 1000 rows on HFSE's Supabase instance; new helper `lib/supabase/paginate.ts::fetchAllPages` walks via `.range()`, applied to attendance/markbook/SIS drill loaders. Earlier passes (necessary-not-sufficient): drill API scope-downgrade when range params missing across 5 module routes; fix records `ENROLLED_STATUSES = ('active','late_enrollee')` (KD #68 mismatch); admissions 'applications' target widened; Records/P-Files narrowed to enrolled-only (KD #51, #71); migrations 040–041. Two follow-ups landed this session: (1) `f183a77` repaired the last unfixed pagination call site — `lib/markbook/drill.ts::loadSheetRowsUncached` chunks the `grade_entries` rollup IN-clause at 200 (matches `loadEntryRowsUncached`); same commit expanded `app/api/sis/drill/[target]/route.ts` ALLOWED_ROLES to include registrar/admissions/p-file — the route was 403'ing every operational consumer of `<DocumentChaseQueueStrip>` per KD #74. (2) `08ff63f` fixed nine bugs on `/markbook/grading/new`: terms scoped to current AY (was mixing AY9999/AY2026/AY2027 — the user-visible symptom that triggered this audit); page role gate via `getSessionUser()` per KD #35 (registrar+superadmin only — proxy.ts admits more); explicit "no current AY" error page replacing the silent fake-UUID fallback; subjects ordered by name; `as never` cast removed; per-query error logging; default term derived server-side from `terms.start_date/end_date` containing today (falls back to `is_current` flag, then term 1); `Level.level_type` typedef extended with `'preschool'` (migration 029); `qa_total` auto-fills from `subject_configs.qa_max` (migration 021) when (section × subject) is picked. **Dev DB state:** AY2026/AY2027 operational data cleared via one-off `scripts/clear-ay2026-ay2027-operational.sql` (uncommitted, untracked); admissions intake tables (`ay{YYYY}_*` trio + `discount_codes`) preserved; `is_current` flipped to AY9999 (KD #52 test env). Sprint 33 + earlier shipped on `main` through `20ac939`. Build clean.

**Future work (post-demo):**
- Net-new charts/KPIs flagged during purpose-alignment audit but not built: Records "Stage not moved in ≥30d" name list; Markbook "Teachers with incomplete sheets" name list; Evaluation "Advisers with zero writeups" name list; Attendance "Chronic absentees ≥10% of school days" headline count; Admissions "Conversion rate by referral source." Each requires a new helper + chart component.
- Two-view split refactor (KD #57): extract Markbook + Evaluation page-level inline conditionals into `components/<module>/<module>-{teacher,registrar}-view.tsx`. Pure code organisation, no behaviour change. Revisit when either page crosses ~800 lines.

**Recent sprints** (full history in `docs/sprints/development-plan.md`; per-pass detail in `git log`):
- Sprint 33 (2026-05-05, `00181dd` + `af3a3c0`): RBAC consolidation. Dropped `admin` role from `Role` union — was a functional twin of `school_admin`. Migration 039 flips live `admin` users to `school_admin` and refreshes `is_registrar_or_above()` to include `school_admin`. School_admin inherits everything `admin` had: grade-change approver pool eligibility (sole pool member; superadmin still excluded as the manager-not-actor), Markbook Change Requests inbox, audit-log export, and access to all `/sis/admin/*` config sub-routes (users, settings, template, subjects, school-config, evaluation-checklists, approvers) that were previously superadmin-only. Superadmin keeps full access for break-glass; day-to-day SIS Admin config now belongs to school_admin. ~70 files touched (TS role-string sweep across `requireRole(...)` arrays, page-level guards, sidebar registry, role-label maps); zero `'admin'` / `"admin"` tokens remain in `app/`, `lib/`, `components/`. KDs updated: #2 (role list), #39 (rewritten — admin retired, school_admin = consolidated generalist), #41 (approver pool now school_admin), #74 (oversight gate prose). Migration 039. Follow-on `af3a3c0`: `lib/sis/seeder/demo-extras.ts` wired into `seedPopulated` as 10th pass — fills the AY9999 fixture with 3 published report cards, 5 parent auth accounts, P-File outreach + 60d/90d expiry buckets, evaluation lifecycle (T1 closed / T2 open + partial / T3 unopened) + PTC, typed calendar events + audience overrides + tentative flag. Tooling-only; no schema changes.
- Sprint 32 (2026-05-04, `d6d6aba`): opt-in comparison (`RangeInput.cmpFrom/cmpTo: string | null`, `RangeResult.comparison/delta/comparisonRange` nullable, dashboards default to current-only, "Add comparison" button surfaces in `DateRangePicker`); AY-switchers clear stale `from/to/cmpFrom/cmpTo` so AY changes don't carry prior-AY dates into drill-downs; report-card page enrolment selection is transfer-aware + unions data across all enrolment rows + denominator from `school_calendar`; attendance grid uses `getDedupedSchoolCalendarForTerm` so primary/secondary precedence yields one row per date; module-dashboard audit fixes (attendance day-type donut audience-aware, records late-enrollee aggregate, evaluation virtue-theme NULL warning); AY-window helper derives Jan–Nov from AY code (KD #13); plain-English UI copy audit (drops "audit-logged on save", "transaction", "upsert", "soft delete", etc.; trust-strip footers reshaped to "AY · count · Refreshes every 10 minutes"); early-bird AY pipeline with `accepting_applications` flag, `/admissions/upcoming/applications` route, AY-Setup wizard toggle + AY-list row toggle, discount-code dialog AY picker. Migration 038; KD #77.
- Sprint 31 (2026-05-04, `d6d6aba`): calendar audience scope + typed event categories + tentative-flag carry-forward + AY-label single-year clarification. Migration 037; KD #76 + extended KDs #13 / #50.
