...
**Current state (2026-05-19):** Sprint 38+ — post-demo production refinement. Migrations 050–054 shipped: (050) `stpApplicationStatus` column + STP doc slots removed from code (KDs #61, #96); (051) `hbl_overlay` on `school_calendar` (KD #98); (052a) audit-log query indexes; (052b) `sync_grading_sheets_from_config` RPC (KD #99); (053) `annual_letter_grade` on `grade_entries` (KD #100); (054) school_config letterhead fields (KD #101). New surfaces: `/admissions/feedback` analytics (KD #102), `/admissions/cohorts/pre-course`, per-module audit-log pages for all 7 modules, PTC feedback route + term resolver (KD #103). Discount codes relocated to `/sis/admin/discount-codes` (legacy redirect at `/records/discount-codes`). **Dev DB state**: all migrations through 054 applied. Build clean.

**Future work:** Non-examinable letter entry UI (`annual_letter_grade` column + route shipped in migration 053; masterfile grid integration + Joann legend confirmation pending — 4 open questions per KD #95 Phase 2); Honors tiers; Sec 4 Economics card; T4 GA row render verification; self-serve invite flow (KD #87 with `/auth/setup`); per-row overflow menus; attendance audit-log server-side pagination; cron auto-sync trigger (KD #90); VL bulk import parser (KD #94 follow-up).

**Recent sprints** (full history in `docs/sprints/development-plan.md`; per-pass detail in `git log`):
- Sprint 38+ (2026-05-15 → 19): post-demo refinement — migrations 050–054 + admissions feedback + audit-log coverage + letterhead config + HBL overlay + annual letter + PTC feedback (KDs #98–103).
