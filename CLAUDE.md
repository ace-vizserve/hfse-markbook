...
**Current state (2026-05-19):** Sprint 38+ — post-demo production refinement. Migrations 050–056 shipped: (050) `stpApplicationStatus` + STP doc slots removed (KDs #61, #96); (051) `hbl_overlay` on `school_calendar` (KD #98); (052a) audit-log indexes; (052b) `sync_grading_sheets_from_config` RPC (KD #99); (053) `annual_letter_grade` on `grade_entries` (KD #100); (054) school_config letterhead fields (KD #101); (055) non-examinable WW/PT/QA score entry — `<LetterGradeGrid>` deleted, `<ScoreEntryGrid letterDisplay>` unified (KD #104); (056) grading-sheet slot-label UX. New surfaces: `/admissions/feedback` analytics (KD #102), `/admissions/cohorts/pre-course`, per-module audit-log pages for all 7 modules, PTC feedback route + term resolver (KD #103). Discount codes relocated to `/sis/admin/discount-codes`. `annual_letter_grade` edit moved to report card page (inline `<AnnualLetterInput>` popover in Final Grade column, print-hidden, registrar/school_admin/superadmin only). Publish-readiness checklist enhancements: correctness fix for T4 quarterly-grade loop (students with zero entries were silently passing); 3 new rows — non-examinable T4 completeness, letterhead populated, virtue theme set (T1–T3 only); adviser-comments row now shows "submitted · drafted · missing" with amber warning on drafted > 0. **Dev DB state**: all migrations through 056 applied. TypeScript clean (`npx tsc --noEmit`); build blocked by Google Fonts network in this env (CI unaffected).

**Future work:** Honors tiers; Sec 4 Economics card; T4 GA row render verification; self-serve invite flow (KD #87 with `/auth/setup`); per-row overflow menus; attendance audit-log server-side pagination; cron auto-sync trigger (KD #90); VL bulk import parser (KD #94 follow-up); Joann legend confirmation for non-examinable letter values (UG/INC/CO/E usage at HFSE).
