...
**Current state (2026-05-05):** Sprint 33 shipped on `main` (`00181dd`) — RBAC consolidation (drop `admin` role, merge into `school_admin`; school_admin gains all `/sis/admin/*` config sub-routes that were superadmin-only; superadmin retained as break-glass + IT). 6 roles total (`teacher | registrar | school_admin | superadmin | p-file | admissions`). Migration 039 applied. Plus follow-on `af3a3c0` — AY9999 demo-extras seeder (3 published cards + parent accounts + P-File outreach + 60d/90d expiry buckets + evaluation lifecycle + typed calendar events) so dashboard demos look populated. Sprints 31–32 (migrations 037, 038, KDs #76, #77) shipped earlier in `d6d6aba`. Build clean.
