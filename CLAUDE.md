...
**Current state (2026-05-24):** Sprint 41 — SOW hard gate + AY readiness indicator. Migrations 059–060 shipped: (059) `curriculum_track` on `template_sections` + `apply_template_to_ay` RPC fix; (060a) `sow_class_instances.has_partial_rebaseline` + `create_grading_sheets_for_scopes` RPC; (060b) `sow_subject_scopes` table seeded for P1–P6 + S1–S4. SOW builder simplified to section-scoped single Save & Apply flow; Subject Scope Manager at `/sis/admin/sow`. AY Readiness Pill: 5-step readiness indicator (AY Setup → Calendar → Sections → SOW → Grading Sheets) mounted in SIS layout, visible to school_admin+ (KD #109). SIS hub redesigned with Year Setup section + numbered AdminCards. Evaluation-checklists admin page removed — topics now fully SOW-sourced (KD #108). **Dev DB state**: migrations through 060 applied. TypeScript clean; build blocked by Google Fonts network (CI unaffected).

**Future work:** Honors tiers; Sec 4 Economics card; T4 GA row render verification; self-serve invite flow (KD #87 with `/auth/setup`); per-row overflow menus; attendance audit-log server-side pagination; cron auto-sync trigger (KD #90); VL bulk import parser (KD #94 follow-up); Joann legend confirmation for non-examinable letter values (UG/INC/CO/E). **Pending Wynne/Joann clarifications**: (1) who encodes PTC comments — teacher or registrar? (2) grade proration formula for missing terms (Wynne). PTC comment encoding surface deferred pending #1.
...
- Sprint 38 (2026-05-13 → 14): workflow hardening + chronic-gap recovery + leave-quota + Masterfile (KDs #88–95; migrations 044–049).
...
- Sprints 28–36 (2026-04-29 → 2026-05-07): see development-plan.md. Highlights: KDs #63–83 + migrations 030–043.
