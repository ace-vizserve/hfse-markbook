...
**Current state (2026-05-06):** Sprint 34 shipping pre-demo — drill-down empty-data fixes + dashboard perf + purpose-alignment cleanup. Latest pass (`f73b448`): root cause of the universal "drill empty across all AYs" symptom turned out to be PostgREST silently capping single-query responses at 1000 rows on the HFSE Supabase instance. AY9999 attendance_daily has 12,200 rows but `loadDailyRowsUncached` was returning 1000 — so most students appeared to have no attendance data and every drill downstream of those loaders looked empty regardless of fixed schema/scope/filter logic. New shared helper `lib/supabase/paginate.ts::fetchAllPages` walks results via `.range()` until the server returns fewer than `pageSize` rows; applied to `lib/attendance/{dashboard,drill}.ts` (attendance_daily), `lib/markbook/{dashboard,drill}.ts` (grade_entries grade-distribution + grade-velocity + drill loader + sheet-rollup), `lib/sis/drill.ts` (section_students). Earlier passes in this sprint (still valid, but were necessary-not-sufficient): drill API scope-downgrade (records/markbook/attendance/admissions/p-files routes — when scope='range' but range params missing, downgrade to 'ay' so target filters get a clean signal); fix records ENROLLED_STATUSES = ('active','late_enrollee') (was ('active','conditional') — 'conditional' isn't a real value, silently dropped late-enrollees from drills, KD #68 mismatch); admissions 'applications' target widened to match the card aggregator. Documented attendance compassionate-quota as AY-wide-by-design + measured AY9999 attendance loader at ~335ms cold-load. Two-view-split refactor (KD #57) deferred as architectural-only. Sprint 33 + earlier shipped on `main` through `20ac939`. Build clean.
