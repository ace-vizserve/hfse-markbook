...
| `key-decisions/markbook.md` | 3, 4, 5, 6, 25, 27, 28, 75, 88, 95, 99, 100, 104, 105, 108, 110 | Grading formula, weights, change-requests, report cards, publishing checklist, change-request hardening, Masterfile + Subject/Overall awards, grading-sheet config sync RPC, non-examinable annual letter grade, non-examinable WW/PT/QA score entry, slot metadata (date + page#), SOW model (infrastructure + teacher-owned model) |
| `key-decisions/evaluation.md` | 49, 92, 93, 103, 106, 107 | Form-class-adviser writeups, per-topic 1–5 ratings, teacher-owned topics, PTC feedback + term resolver, subject-teacher picker nav, evaluation topic ownership (superseded by KD #110) |
...
| `key-decisions/records.md` | 37, 40, 48, 66, 67, 68, 72, 83, 85, 90, 101, 109 | Records + SIS Admin: AY rollover, master template, section transfers, late-enrollee, movements, re-alphabetize, unsynced students, dynamic letterhead config, AY readiness indicator |
