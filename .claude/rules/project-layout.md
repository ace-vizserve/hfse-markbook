...
components/                      ui (shadcn) + per-module folders + shared `module-sidebar.tsx` + `module-sidebar/{sidebar-header,sidebar-nav-item,sidebar-profile,sidebar-quick-action}.tsx` (KD #58 — replaces the 8 per-module `<module>-sidebar.tsx` files + the topbar `<ModuleSwitcher>`) + `topbar-module-switcher.tsx` (used only on the neutral `/` + `/account` shells, where there is no module sidebar to host the switcher); components/dashboard/* shared dashboard UI — DashboardHero, ComparisonToolbar, MetricCard (with `drillSheet` slot), InsightsPanel, ActionList, PriorityPanel (operational top-of-fold per KD #57), DrillDownSheet (universal toolkit), ChartLegendChip (gradient pill per `09a-design-patterns.md` §10), charts/{trend,comparison-bar,donut,sparkline}-chart.tsx; components/<module>/drills/{<module>-drill-sheet,chart-drill-cards}.tsx for drill wrappers
